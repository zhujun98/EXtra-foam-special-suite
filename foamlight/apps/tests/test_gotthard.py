...
from foamlight.apps.gotthard import (
    Gotthard, GotthardProcessor, GotthardImageView, GotthardAvgPlot,
    GotthardPulsePlot, GotthardHist,
    _DEFAULT_N_BINS, _DEFAULT_BIN_RANGE, _PIXEL_DTYPE,
)
...
class TestGotthardApp(_AppTestBase):
    @classmethod
    def setUpClass(cls):
        cls._app = Gotthard('MID')
...
class TestGotthardProcessor(_RawDataMixin, _ProcessorTestBase):
    ...
    def _check_processed_data_structure(self, ret):
        """Override."""
        data_gt = TestGotthardApp.data4visualization().keys()
