...
from foamlight.apps.xas_tim import (
    XasTim, XasTimProcessor, XasTimXgmPulsePlot, XasTimDigitizerPulsePlot,
    XasTimMonoScanPlot, XasTimCorrelationPlot, XasTimSpectraPlot, XasTimXgmSpectrumPlot,
...
class TestXasTimApp(_AppTestBase):
    ...
    def testWidgets(self):
        self.assertEqual(9, len(self._app._plot_widgets_st))
        counter = Counter()
    ...
    def testCtrl(self):
        ...
        for i, widget in enumerate(ctrl_widget.spectra_displayed.buttons()):
            ...
            QTest.mouseClick(widget, Qt.LeftButton, pos=QPoint(2, widget.height()/2))
            self.assertEqual(displayed_gt, self._app._spectra._displayed)


class TestXasTimProcessor(_RawDataMixin, _ProcessorTestBase):
    ...
    def _check_processed_data_structure(self, ret):
        """Override."""
        data_gt = TestXasTimApp.data4visualization().keys()
