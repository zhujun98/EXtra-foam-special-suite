...
class _CommonCtrlWidgetS(QFrame):
    """Control widget used in all applications.
    ...
    def addRoiCtrl(self, roi):
        ...
        self.roi_ctrl = SingleRoiCtrlWidget(roi, with_lock=False)
...
class _FoamLightApp(QMainWindow):
    """Base class for a Qt application.

    It should be inherited by all concrete windows.
    """

    _TOTAL_W, _TOTAL_H = config["APP_WINDOW_SIZE"]
    ...
    def __init__(self, topic, **kwargs):
        ...
        self.setWindowTitle(f"foamlight {__version__} - {self._title}")

        self._com_ctrl_st = _CommonCtrlWidgetS(**kwargs)

        cv = Condition()
        queue = SimpleQueue(maxsize=1)
        self._client_st = self._client_instance_type(queue, cv)
        ...
        self._plot_timer_st.setInterval(config["APP_PLOT_UPDATE_TIMER"])
        self._plot_timer_st.timeout.connect(self.updateWidgetsST)

        self._initUIST()
        self._initConnectionsST()
    ...
    def _initConnectionsST(self):
        # init Connections
        com = self._com_ctrl_st
        client = self._client_st
        worker = self._worker_st

        # logging
        client.log.logOnMainThread(self)
        worker.log.logOnMainThread(self)

        # start/stop/reset
        com.start_btn.clicked.connect(self._onStartST)
        com.stop_btn.clicked.connect(self._onStopST)
        com.reset_btn.clicked.connect(self._onResetST)

        # dark operation
        com.record_dark_btn.toggled.connect(worker.onRecordDarkToggledST)
        com.record_dark_btn.toggled.emit(com.record_dark_btn.isChecked())
        com.load_dark_run_btn.clicked.connect(self._onSelectDarkRunDirectoryST)
        com.remove_dark_btn.clicked.connect(worker.onRemoveDark)
        com.dark_subtraction_cb.toggled.connect(worker.onSubtractDarkToggledST)
        com.dark_subtraction_cb.toggled.emit(com.dark_subtraction_cb.isChecked())

        # auto-level
        com.auto_level_btn.clicked.connect(self._onAutoLevelST)

        # ROI ctrl
        com.roi_geometry_change_sgn.connect(worker.onRoiGeometryChange)
