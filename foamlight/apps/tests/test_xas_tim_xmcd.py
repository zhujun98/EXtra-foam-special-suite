...
from foamlight.apps.xas_tim_xmcd import (
    XasTimXmcd, XasTimXmcdProcessor, XasTimXgmPulsePlot, XasTimDigitizerPulsePlot,
    XasTimXmcdSlowScanPlot, XasTimCorrelationPlot, XasTimXmcdAbsorpPnSpectraPlot,
...
class TestXasTimXmcdApp(_AppTestBase):
    @classmethod
    def setUpClass(cls):
        cls._app = XasTimXmcd('SCS')
    ...
    def testWidgets(self):
        self.assertEqual(10, len(self._app._plot_widgets_st))
        counter = Counter()
    ...
    def testCtrl(self):
        ...
        for i, widget in enumerate(ctrl_widget.spectra_displayed.buttons()):
            ...
            self.assertEqual(i, self._app._pn_spectra._displayed)
            self.assertEqual(i, self._app._xas_xmcd_spectra._displayed)


class TestXasTimXmcdProcessor(_RawDataMixin, _ProcessorTestBase):
    ...
    def _check_processed_data_structure(self, ret):
        """Override."""
        data_gt = TestXasTimXmcdApp.data4visualization().keys()
